...
public class MlHospitalDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ml_hospital_detail);
...
// 3. 병원 이름, 별점, 찜 버튼 설정
        ...
        textHospitalName.setText("일미리 성형외과");
        textRating.setText("★ 8.6");


        // 병원 정보 정의 (이름 + 배너 이미지 ID)
        HospitalData amondHospital = new HospitalData("일미리 성형외과", R.drawable.ml_1);
...
// 지도 이미지 클릭 시 구글맵 이동
        imageMapPreview.setOnClickListener(v -> {
            Uri gmmIntentUri = Uri.parse("geo:0,0?q=서울 서초구 강남대로 449 (서초동, 영신빌딩) 영신빌딩 7,8,9,10층");
        ...
        List<Integer> imageResIds = Arrays.asList(
                R.drawable.ml_1, R.drawable.ml_2, R.drawable.ml_3
        ...
        List<DoctorData> doctors = Arrays.asList(
                new DoctorData(
                        "김홍진",
                        R.drawable.doctor_ml_park,
                        Arrays.asList("눈성형", "코성형","지방성형","필러")
                )
        );
        ...
        showMore.setOnClickListener(v -> {
            ...
            Intent intent = new Intent(MlHospitalDetailActivity.this, ReviewMoreActivity.class);
            intent.putExtra("hospitalName", "일미리성형외과");
