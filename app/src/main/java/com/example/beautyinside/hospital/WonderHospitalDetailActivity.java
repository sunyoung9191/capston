...
public class WonderHospitalDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_wonder_hospital_detail);
        ...
        showMore.setOnClickListener(v -> {
            ...
            Intent intent = new Intent(WonderHospitalDetailActivity.this, ReviewMoreActivity.class);
            intent.putExtra("hospitalName", "원더풀성형외과");
        ...
        List<DoctorData> doctors = Arrays.asList(
                new DoctorData(
                        "구윤택",
                        R.drawable.doctor_wonder_park,
                        Arrays.asList("눈성형", "코성형","리프팅","가슴성형")
                )
        );
...
// 지도 이미지 클릭 시 구글맵 이동
        imageMapPreview.setOnClickListener(v -> {
            Uri gmmIntentUri = Uri.parse("geo:0,0?q=서울 서초구 강남대로 419 (서초동, 파고다타워) 9층 원더풀 성형외과");
        ...
        textHospitalName.setText("원더풀 성형외과");
        textRating.setText("★ 9.4");

        // 병원 정보 정의 (이름 + 배너 이미지 ID)
        HospitalData amondHospital = new HospitalData("원더풀 성형외과", R.drawable.wonder_1);
...
// 하트 클릭 시 동작
        ...
        List<Integer> imageResIds  = Arrays.asList(
                R.drawable.wonder_1, R.drawable.wonder_2
